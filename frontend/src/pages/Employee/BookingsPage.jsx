...
const [showTicketPopup, setShowTicketPopup] = useState(false); // To control popup visibility
  ...
  const fetchBookings = async () => {
    try {
      const response = await axios.get("http://cinema-backend:3000/employee/bookings");
  ...
  const fetchMovies = async () => {
    try {
      ...
      const response = await axios.get("http://cinema-backend:3000/api/movies");
  ...
  const handleShowtimeSelect = async (showtime) => {
    ...
    try {
      const response = await axios.post("http://cinema-backend:3000/employee/check-available-seats", {
  ...
  const handleAddBooking = async () => {
    ...
    try {
      await axios.post("http://cinema-backend:3000/employee/bookings/addbooking", {
        email: userEmail,
        movieId: selectedMovie,
  ...
  const handleUpdateBooking = async () => {
    ...
    try {
      await axios.put(`http://cinema-backend:3000/employee/bookings/${selectedBooking._id}`, {
  ...
  const handlePrintTicket = async (bookingId) => {
    try {
      const response = await axios.get(
        `http://cinema-backend:3000/employee/bookings/print/${bookingId}`
      );
