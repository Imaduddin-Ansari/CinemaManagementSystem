...
export const ShiftsPage = () => {
  ...
  useEffect(() => {
    const fetchShifts = async () => {
      try {
        const response = await axios.get('http://cinema-backend:3000/api/shift-management/shifts');
  ...
  const handleRequestShiftChange = async (shiftId) => {
    try {
      const response = await axios.post('http://cinema-backend:3000/api/shift-management/shifts/request-change', {
        shiftId,
  ...
  const handleViewShiftDetails = async (shiftId) => {
    try {
      const response = await axios.get(`http://cinema-backend:3000/api/shift-management/shifts/${shiftId}`);
